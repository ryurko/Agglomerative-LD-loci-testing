...
save_adapt_snps_gene_tables <-
  function(assign_type, phenotype, rsquared, target_alpha = 0.05) {
    ...
    adapt_results <-
      readRDS(
        dir(paste0("data/adapt_results/",
                   assign_type),
            full.names = TRUE,
            pattern = paste0(phenotype, "_rsquared", rsquared,
                             "_xgb(_cv)?\\.rds"))
      )
    ...
    adapt_gene_ld_loci_table <- gene_ld_loci_table %>%
      filter(ld_loci_id %in% adapt_ld_loci)

    adapt_snp_gene_table <- snp_ld_loci_table %>%
      filter(ensembl_id %in% adapt_gene_ld_loci_table$ensembl_id)
    ...
    write_csv(adapt_snp_gene_table,
              paste0("data/adapt_results/",
                     assign_type, "/snp_gene_tables/",
                     phenotype, "_rsquared", rsquared, "_snp_gene_table.csv"))
