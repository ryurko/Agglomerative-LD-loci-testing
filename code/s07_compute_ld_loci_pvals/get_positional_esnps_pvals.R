...
#          the Positional + eSNPs based assignments
...
tidy_snp_ld_loci_data <-
  read_csv("data/tidy_snp_ld_loci/rsquared25/positional_esnps.csv") %>%
...
for (chr_i in all_chr) {
  ...
  write_csv(tidy_ld_loci_summary,
            paste0("data/ld_loci_level_pvals/rsquared25/positional_esnps/chr",
                   chr_i, "_summary.csv"))
...
tidy_snp_ld_loci_data <-
  read_csv("data/tidy_snp_ld_loci/rsquared50/positional_esnps.csv") %>%
  dplyr::select(-hg38_id, -hg38_chr, -chr)
...
for (chr_i in all_chr) {
  ...
  write_csv(tidy_ld_loci_summary,
            paste0("data/ld_loci_level_pvals/rsquared50/positional_esnps/chr",
                   chr_i, "_summary.csv"))
...
# Load the tidy SNP gene-set data:
tidy_snp_ld_loci_data <-
  read_csv("data/tidy_snp_ld_loci/rsquared75/positional_esnps.csv") %>%
  dplyr::select(-hg38_id, -hg38_chr, -chr)
...
for (chr_i in all_chr) {
  ...
  write_csv(tidy_ld_loci_summary,
            paste0("data/ld_loci_level_pvals/rsquared75/positional_esnps/chr",
                   chr_i, "_summary.csv"))
