...
pos_esnps_pval_data <-
  map_dfr(list.files("data/ld_loci_level_pvals/asd/rsquared25/positional_esnps",
                     full.names = TRUE),
          function(file_name) read_csv(file_name))

pos_only_pval_data <-
  map_dfr(list.files("data/ld_loci_level_pvals/asd/rsquared25/positional",
                     full.names = TRUE),
          function(file_name) read_csv(file_name))
...
tidy_pos_esnps_gene_table <-
  read_csv("data/tidy_gene_ld_loci/asd/positional_esnps/agglom_rsquared25_gene_sets.csv")

tidy_pos_gene_table <-
  read_csv("data/tidy_gene_ld_loci/asd/positional/agglom_rsquared25_gene_sets.csv")
...
pos_esnps_results_data <- pos_esnps_pval_data %>%
  ...
  mutate(scz_z = qnorm(1 - scz_vegas_pval),
         asd_z = qnorm(1 - asd_vegas_pval),
         ea_z = qnorm(1 - ea_vegas_pval),
         cap_scz_vegas_pval = pmax(1e-15, pmin(1 - 1e-15, scz_vegas_pval)),
         cap_asd_vegas_pval = pmax(1e-15, pmin(1 - 1e-15, asd_vegas_pval)),
         cap_ea_vegas_pval = pmax(1e-15, pmin(1 - 1e-15, ea_vegas_pval)),
         cap_scz_z = qnorm(1 - cap_scz_vegas_pval),
         cap_asd_z = qnorm(1 - cap_asd_vegas_pval),
         cap_ea_z = qnorm(1 - cap_ea_vegas_pval))
...
make_phenotype_long <- . %>%
  dplyr::select(ld_loci_id, length_size, cap_asd_z, cap_scz_z, cap_ea_z) %>%
...
names(pos_esnps_unadj_asd_z) <- pos_esnps_results_data$ld_loci_id
pos_only_unadj_asd_z <- pos_only_results_data$cap_asd_z
names(pos_only_unadj_asd_z) <- pos_only_results_data$ld_loci_id
...
names(pos_esnps_adj_asd_z) <- pos_esnps_results_data$ld_loci_id
pos_only_adj_asd_z <- pos_only_results_data$adj_asd_z
names(pos_only_adj_asd_z) <- pos_only_results_data$ld_loci_id
...
pos_esnps_asd_adj_gsea %>%
  ...
  write_csv("data/gsea/output/asd_positional_esnps_results.csv")

pos_only_asd_adj_gsea %>%
  make_gsea_table %>%
  mutate(phenotype = "ASD", positional_type = "Positional") %>%
  dplyr::select(phenotype, positional_type, everything()) %>%
  write_csv("data/gsea/output/asd_positional_results.csv")
...
names(pos_esnps_unadj_scz_z) <- pos_esnps_results_data$ld_loci_id
pos_only_unadj_scz_z <- pos_only_results_data$cap_scz_z
names(pos_only_unadj_scz_z) <- pos_only_results_data$ld_loci_id
...
names(pos_esnps_adj_scz_z) <- pos_esnps_results_data$ld_loci_id
pos_only_adj_scz_z <- pos_only_results_data$adj_scz_z
names(pos_only_adj_scz_z) <- pos_only_results_data$ld_loci_id
...
pos_esnps_scz_adj_gsea %>%
  ...
  dplyr::select(phenotype, positional_type, everything()) %>%
  write_csv("data/gsea/output/scz_positional_esnps_results.csv")

pos_only_scz_adj_gsea %>%
  make_gsea_table %>%
  mutate(phenotype = "SCZ", positional_type = "Positional") %>%
  dplyr::select(phenotype, positional_type, everything()) %>%
  write_csv("data/gsea/output/scz_positional_results.csv")
...
names(pos_esnps_unadj_ea_z) <- pos_esnps_results_data$ld_loci_id
pos_only_unadj_ea_z <- pos_only_results_data$cap_ea_z
names(pos_only_unadj_ea_z) <- pos_only_results_data$ld_loci_id
...
names(pos_esnps_adj_ea_z) <- pos_esnps_results_data$ld_loci_id
pos_only_adj_ea_z <- pos_only_results_data$adj_ea_z
names(pos_only_adj_ea_z) <- pos_only_results_data$ld_loci_id
...
pos_esnps_ea_adj_gsea %>%
  ...
  dplyr::select(phenotype, positional_type, everything()) %>%
  write_csv("data/gsea/output/ea_positional_esnps_results.csv")

pos_only_ea_adj_gsea %>%
  make_gsea_table %>%
  mutate(phenotype = "EA", positional_type = "Positional") %>%
  dplyr::select(phenotype, positional_type, everything()) %>%
  write_csv("data/gsea/output/ea_positional_results.csv")
