# PURPOSE: Determine the GCV bandwidths for the loci for each set of results
...
get_gcv_bw <- function(snp_ld_loci_results) {
  ...
  # Assume input dataset has the ld_locus_id, asd_z_squared, and bp columns:
  map_dfr(unique(snp_ld_loci_results$ld_locus_id),
          function(ld_loci_i) {
            ...
            if (nrow(ld_loci_data) > 2) {
              ld_loci_bw0 <- npregbw(asd_z_squared ~ bp, data = ld_loci_data)$bw
