# PURPOSE: Initialize fake SCZ tuning results the Positional + eSNPs data
...
model_settings <- expand.grid("nrounds" = seq(100, 500, by = 50),
                              "max_depth" = c(1, 2),
                              "eta" = seq(0.02, 0.06, by = 0.01),
                              "gamma" = c(0)) %>%
  as_tibble()

# Now create the results in parallel for SCZ
scz_results <- mclapply(1:nrow(model_settings), mc.cores = 15,
...
readr::write_csv(scz_results,
                 "data/adapt_results/positional_esnps/fake_scz_tune/rsquared25_tune_results.csv")
...
# Save the dataframe summarizing results:
readr::write_csv(scz_results,
                 "data/adapt_results/positional_esnps/fake_scz_tune/rsquared50_tune_results.csv")
                          ...
                          do.call(rbind,
                                  lapply(target_alphas,
                                         function(alpha) {
                                           ...
                                           # Return the fdp and power:
                                           return(data.frame("phenotype" = "fake_scz",
                                                             "ld_loci_type" = "pos_esnps_rsquared75",
                                                             "method" = "adapt_xgboost",
                                                             "variables" = "all",
                                                             "alpha" = alpha,
                                                             "n_disc" = length(adapt_disc),
...
# Save the dataframe summarizing results:
readr::write_csv(scz_results,
                 "data/adapt_results/positional_esnps/fake_scz_tune/rsquared75_tune_results.csv")
