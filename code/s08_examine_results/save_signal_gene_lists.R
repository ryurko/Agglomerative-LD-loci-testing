...
save_signal_gene_lists <-
  function(assign_type, phenotype, rsquared, include_functional = TRUE) {
    ...
    ld_loci_signals_data <- ld_loci_signals_data %>%
      ...
      separate(ld_locus_id, c("ld_loci_chr", "set_id"), sep = "_", remove = FALSE) %>%
    ...
    ld_loci_gene_info_data <- gene_info_data %>%
      ...
      filter(ld_locus_id %in% unique(ld_loci_signals_data$ld_locus_id))
    ...
    non_signal_genes <- gene_info_data %>%
      filter(ld_locus_id %in%
               setdiff(gene_info_data$ld_locus_id,
                       ld_loci_signals_data$ld_locus_id)) %>%
      pull(ensembl_id)
    ...
    signal_gene_file <-
      file(
        paste0("data/adapt_results/", assign_type,
              "/gene_lists/signals/", phenotype, "_rsquared", rsquared,
