...
pos_esnps_ld_loci_level_smoothing <-
  get_ld_loci_level_smooth_results(pos_esnps_loc_snp_ld_loci_results, pos_esnps_bw_data,
                              "asd_z_squared", n_ld_loci_points = 1000)
...
get_ld_loci_level_scz_ea_background <- function(snp_ld_loci_data, bw_data, n_ld_loci_points = 1000) {
  ...
  map_dfr(unique(snp_ld_loci_data$ld_loci_id),
          function(ld_loci_i) {
            ...
            if (is.na(ld_loci_bw)) {
              ...
              smooth_results <- tibble(bp = ld_loci_data$bp,
                                       smooth_scz_background = ld_loci_data[["adj_scz_z_squared"]],
                                       smooth_ea_background = ld_loci_data[["adj_ea_z_squared"]])
            } else {
              
              scz_smooth_results <-
                get_smooth_interpolation(ld_loci_data$bp,
                                         ld_loci_data[["adj_scz_z_squared"]],
                                         ld_loci_bw, 1000) %>%
                dplyr::rename(smooth_scz_background = smooth_zsquared)
              ea_smooth_results <-
                get_smooth_interpolation(ld_loci_data$bp,
                                         ld_loci_data[["adj_ea_z_squared"]],
                                         ld_loci_bw, 1000) %>%
...
pos_esnps_ld_loci_background <-
  get_ld_loci_level_scz_ea_background(pos_esnps_loc_snp_ld_loci_results, pos_esnps_bw_data,
                                 n_ld_loci_points = 1000)
pos_ld_loci_background <-
  get_ld_loci_level_scz_ea_background(pos_loc_snp_ld_loci_results, pos_bw_data,
                                 n_ld_loci_points = 1000)
