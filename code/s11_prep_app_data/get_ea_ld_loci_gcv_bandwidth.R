...
get_gcv_bw <- function(snp_ld_loci_results) {
  ...
  # Assume input dataset has the ld_loci_id, ea_z_squared, and bp columns:
  map_dfr(unique(snp_ld_loci_results$ld_loci_id),
          function(ld_loci_i) {
            ...
            if (nrow(ld_loci_data) > 2) {
              ld_loci_bw0 <- npregbw(ea_z_squared ~ bp, data = ld_loci_data)$bw
...
write_csv(pos_esnps_loc_gcv,
          "data/kernel_smoothing/ea/input/positional_esnps/ld_loci_gcv_bw.csv")
write_csv(pos_loc_gcv,
          "data/kernel_smoothing/ea/input/positional/ld_loci_gcv_bw.csv")
