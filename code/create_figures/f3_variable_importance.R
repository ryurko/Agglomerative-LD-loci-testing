...
pos_esnps_model_data <-
  read_csv("data/adapt_model_data/asd/positional_esnps/rsquared25_model_data.csv")

pos_model_data <-
  read_csv("data/adapt_model_data/asd/positional/rsquared25_model_data.csv")
...
pos_esnps_var_imp_plot <- pos_esnps_var_imp_summary %>%
  ...
  mutate(model_type = ifelse(model_type == "mu", "Non-null effect size",
         ...
         Feature = fct_recode(Feature, `EA z statistics` = "cap_ea_vegas_z",
                              `SCZ z statistics` = "cap_scz_vegas_z",
                              `ASD z statistics` = "cap_asd_vegas_z",
                              `Min. LOEUF` = "min_loeuf",
...
save_plot("figures/main/f3_pos_esnps_importance.jpg",
          pos_esnps_var_imp_plot, ncol = 3, nrow = 1)
save_plot("figures/main/f3_pos_esnps_importance.pdf",
          pos_esnps_var_imp_plot, ncol = 3, nrow = 1)
...
pos_var_imp_plot <- pos_var_imp_summary %>%
  ...
  mutate(model_type = ifelse(model_type == "mu", "Non-null effect size",
         ...
         Feature = fct_recode(Feature, `EA z statistics` = "cap_ea_vegas_z",
                              ...
                              `Min. LOEUF` = "min_loeuf",
                              `Number of SNPs` = "n_snps",
                              `lncRNA indicator` = "biotype_lnc_rna"),
         Feature = tidytext::reorder_within(Feature, total_ave_gain, phenotype)) %>%
  ggplot(aes(x = Feature, y = ave_gain, fill = model_type, color = model_type)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("darkblue", "darkorange"),
...
save_plot("figures/suppl/f_pos_importance.jpg",
          pos_var_imp_plot, ncol = 3, nrow = 1)
save_plot("figures/suppl/f_pos_importance.pdf",
          pos_var_imp_plot, ncol = 3, nrow = 1)
