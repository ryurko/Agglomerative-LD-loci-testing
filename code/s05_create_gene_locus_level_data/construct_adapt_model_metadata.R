# PURPOSE: Initialize the AdaPT modeling datasets for the agglomerative genes/loci

library(tidyverse)
library(data.table)


# Load the gene/locus p-values --------------------------------------------
...
pos_esnps_ld_loci_pval_list <-
  map(rsquared_thresholds,
      function(rsquared_cut) {
        map_dfr(list.files(
          paste0(
            "data/locus_level_pvals/positional_esnps/rsquared",
            rsquared_cut * 100,  "/"),
          full.names = TRUE),
...
pos_ld_loci_pval_list <-
  map(rsquared_thresholds,
      function(rsquared_cut) {
        map_dfr(list.files(
          paste0(
            "data/locus_level_pvals/positional/rsquared",
            rsquared_cut * 100,  "/"),
          full.names = TRUE),
          function(file_name) read_csv(file_name))
      })
...
pos_esnp_gene_ld_loci_list <-
  map(rsquared_thresholds,
      function(rsquared_cut) {
        read_csv(
          paste0(
            "data/tidy_gene_locus/positional_esnps/agglom_rsquared",
...
pos_gene_ld_loci_list <-
  map(rsquared_thresholds,
      function(rsquared_cut) {
        read_csv(
          paste0(
            "data/tidy_gene_locus/positional/agglom_rsquared",
            rsquared_cut * 100, "_ld_loci.csv")
        )})
...
# Create a pipeline for summarizing gene set BrainVar covariates at the gene-set
...
create_brainvar_eqtl_cov <- . %>%
  ...
  ungroup() %>%
  # Replace missings for the variances with 0 (these are due to just 1 SNP):
...
create_wgcna_indicators <- . %>%
  dplyr::select(ld_locus_id, ensembl_id) %>%
...
gtex_fcba9_eqtl_summary <- gtex_fcba9_eqtl_data %>%
  ...
  # Replace missings with 0 since that represents no expression:
...
gtex_cortical_wgcna_data <-
  read_csv("data/eqtls/gtex/wgcna/cortical_wgcna_results.csv")
...
# Also replace the missings for eQTL slopes with 0
...
create_adapt_model_data <- function(gs_pval_summary, gene_gs_table) {

  # First join the gene-type to the gene_gs_table:
  gs_gene_type_summary <- gene_gs_table %>%
    dplyr::left_join(
  ...
  gs_data <- gs_pval_summary %>%
    dplyr::left_join(gs_gene_type_summary, by = "ld_locus_id")

  # join brainvar data:
  gs_model_data <- gs_data %>%
    dplyr::left_join({
      brainvar_eqtl_data %>%
        filter(ensembl_id %in% gene_gs_table$ensembl_id) %>%
        dplyr::left_join(gene_gs_table,
                         by = "ensembl_id") %>%
        create_brainvar_eqtl_cov
    }, by = "ld_locus_id") %>%
    dplyr::left_join(create_wgcna_indicators(gene_gs_table),
                     by = "ld_locus_id") %>%
    # join gtex:
    dplyr::left_join({
      gtex_eqtl_summary %>%
    ...
    dplyr::left_join(create_gtex_wgcna_indicators(gene_gs_table),
                     by = "ld_locus_id") %>%
    dplyr::left_join(make_loeuf_summary(gene_gs_table),
                     by = "ld_locus_id") %>%
    update_eqtl_vars() %>%
    add_z_stat_vars()

  # Add fake p-values
  gs_model_data <- gs_model_data %>%
    arrange(scz_quad_pval) %>%
    mutate(scz_pval_rank = 1:n()) %>%
    left_join(gs_model_data %>% make_fake_scz_pval,
  ...
  # Join to gene-sets then to the model data:
  gs_model_data <- gs_model_data %>%
    dplyr::left_join(eqtl_gene_esnp_counts, by = "ld_locus_id") %>%
  ...
  # More gene counts:
  gs_gene_count <- gene_gs_table %>%
    group_by(ld_locus_id) %>%
    summarize(n_genes = n())

  gs_model_data <- gs_model_data %>%
    dplyr::left_join(gs_gene_count, by = "ld_locus_id") %>%
...
walk(1:length(pos_esnps_model_data_list),
     function(gs_i) {
       write_csv(pos_esnps_model_data_list[[gs_i]],
                 paste0(
                   "data/adapt_model_data/positional_esnps/rsquared",
                   rsquared_thresholds[gs_i] * 100, "_model_data.csv"
                 ))
     })

walk(1:length(pos_model_data_list),
     function(gs_i) {
       write_csv(pos_model_data_list[[gs_i]],
                 paste0(
